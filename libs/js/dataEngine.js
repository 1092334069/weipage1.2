	 ...
	 * 		}
	 * 		例如:
	 * 			let a = {b1:"1",b2:"2",b3:[{c1;"3",c2:"4"},{c1:"5",c2:"6"}]}
	 * 			let x = [{y1:"1",y2:[{z:1:"1",z2:"2"}]},{y1:"11",y2:[{z:1:"11",z2:"21"}]}];
	...
	dataEngine.prototype.searchData = function(count,searchOption,sourceData){		
		...
		let result,search,data,parentData,parentIndex;
		...
		if(Array.isArray(searchOption) && searchOption.length > 0){
			//取出第一个对象
			let searchList = searchOption.splice(0,1);	
			search = searchList[0];
		...
		if(search.value || search.value == 0){
				let valueNum = Number(search.value);				
				if(Array.isArray(data)){					
					...
					if(search.key){
						if(data.hasOwnProperty(search.key)){							
							if(Array.isArray(data[search.key])){							
								if(!isNaN(valueNum)){
	...
	dataEngine.prototype.sizzle = function(selectStr){
		...
		let reg = /\[(.+?)\]/g;
		let selectList = selectStr.split(">");
		let resultList = [];
		
		for(let i = 0; i < selectList.length; i++){
			let data = selectList[i];
			if(data){				
				let key = null,value = null;
				//先取出值
				if(reg.test(data)){
					let tempValue =data.match(reg)[0];
	...
	dataEngine.prototype.$ = dataEngine.prototype.selector = function(select,source){
		...
		if(typeof select == "object"){
			...
			for(let i = 0; i < searchList.length; i++){
				let searchOne = searchList[i];
				for(let key in searchOne){
					selectResult.push({
						key:key,
						value:searchOne[key]
					});
		...
		let res = this.getData(selectResult,source);
		
		return this.constructionResult(res);
	};
	...
	dataEngine.prototype.insertBeforeData = function(dataList,data,param){		
		if(param){
			if(Array.isArray(dataList)){
				let insertIndex = this.getInsertIndex(param,dataList);
	...
	dataEngine.prototype.changeSort = function(dataList,oldIndex,newIndex){
		...
		let data = dataList.splice(oldIndex,1)[0];
	...
	dataEngine.prototype.setData = function(data,obj,deep){
		if(typeof data == "object" && !Array.isArray(data) && typeof obj == "object" && !Array.isArray(obj)){			
	...
	dataEngine.prototype.exportData = function(){
		let copyView = JSON.parse(JSON.stringify(this.view));
		let disableList = this.getDisableProperty();
