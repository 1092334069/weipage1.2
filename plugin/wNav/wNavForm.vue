<template>
	<div>
		...
		<div class="yui_form_wrap">
			...
			<div class="input_wrap">						
				<div class="form_one">
					<input type="radio" name="subPluginType" value="TEXT" :checked="checkTheSame(pluginData.subPluginType,'TEXT')" /> 文字  
					<input type="radio" name="subPluginType" value="IMAGE" :checked="checkTheSame(pluginData.subPluginType,'IMAGE')" /> 图标+文字
				</div>
			</div>
		</div>
		<div class="yui_form_wrap">
			<p class="input_lable">导航列表：</p>
			<div class="input_wrap">
				<span v-for="item in pluginData.subPluginList" class="childPlugin_wrap childPlugin_lable subPlugin_wrap drop_wrap drop_copy_wrap" :class="{current:item.pluginSelect}" :data-plugInUuid="item.pluginUuid">{{item.name}}</span>
