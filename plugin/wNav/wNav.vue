	...
	<div :data-pluginUuid="pluginData.pluginUuid" class="plugin_wrap drop_wrap drop_copy_wrap" :class="parsePluginSelect(pluginData.pluginSelect)" :style="parsePluginStyle(pluginData.backgroundColor,pluginData.backgroundImage)">
		<div v-if="pluginData.subPluginType == 'TEXT'" class="nav_wrap">
	        <ul class="nav_list child_list font_14">
	            <li v-for="item in pluginData.subPluginList" :data-pluginUuid="item.pluginUuid" class="nav_item subPlugin_wrap drop_wrap drop_copy_wrap" :class="{current:item.pluginSelect}" >
	            	<a draggable="false" href="javascript:;" class="block_link">{{parseNavName(item.name)}}</a>
	        	</li>
	        </ul>
	    </div>
